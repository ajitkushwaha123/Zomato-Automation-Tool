...
export const statusOptions = [
  { name: "Veg", uid: "veg" },
  { name: "Non-Veg", uid: "non_veg" },
  { name: "Egg", uid: "egg" },
];

const getStatus = (status) => {
  let foundStatus;
  statusOptions.map((allStatus, index) => {
    if (allStatus.uid == status) {
      foundStatus = allStatus.name;
    }
  });
  return foundStatus;
};
 ...
 const filteredData = (menuData, filters) => {
  ...
  const renderCell = React.useCallback((user, columnKey) => {
    ...
    switch (columnKey) {
        ...
        return (
          <div>
            <button
              ...
              //   `bg-white flex ${
              //     user?.food_type == "veg"
              //       ? "bg-[#16af62] text-[#e7ffe3]"
              //       : user?.food_type === "non_veg"
              //       ? "bg-[#ff7373] text-[#fcc]"
              //       : "text-[#ceaa1b] bg-[#ffe8e8]"
              //   } justify-center items-center text-nowrap px-3 py-1 rounded-full`
              // )}
            >
              {getStatus(user?.food_type)}
            </button>
          </div>
        );
      case "status":
      ...
      case "description":
        return (
          <div className="truncate max-w-[200px]">{user.description}...</div>
        );
      case "item_type":
        return (
          <div
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end">
          ...
          <div className="flex gap-3">
            <Dropdown>
              <DropdownTrigger className="hidden bg-white sm:flex">
                <Button
